...
if(!content.includes('Use Wallet Balance')) {
    ...
    // EXACT REPLACEMENT FOR T1
    const t1 = `                        <div className="pt-3 mt-3 border-t border-pink-200 flex justify-between items-center">
                          <span className="text-gray-800 font-bold">Final Amount</span>`;
    content = content.replace(t1, wallet_coupon_ui);
}
...
if(!content.includes('Have a Referral Code?')) {
    ...
    content = content.replace(t2, referral_input_ui);
}
