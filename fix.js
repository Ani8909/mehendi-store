...
if (!content.includes('applyReferralCode = async')) {
...
if (!content.includes('finalCustomerId = userCredential.user.uid')) {
    ...
    content = content.replace(
        /    \/\/ Final Step - Create Booking \(Bypassing Razorpay for offline payments\)\r?\n\s*setLoading\(true\);\r?\n\s*try \{\r?\n\s*const bRef = "JM-" \+ Math\.random\(\)\.toString\(36\)\.substring\(2, 8\)\.toUpperCase\(\);/g,
        booking_replacement
    );
}
...
if (!content.includes('finalWalletBalance - walletDeduction')) {
    ...
    content = content.replace(
        /      const docRef = await addDoc\(collection\(db, "bookings"\), bookingData\);\r?\n\s*\/\/ Send Email Notification/g,
        after_booking_insert
    );
    content = content.replace('email: user?.email || ""', 'email: finalUserEmail');
    content = content.replace(
        /router\.push\(\`\/booking-slip\/\$\{docRef\.id\}\`\);/g,
...
if (!content.includes('Create Password')) {
    ...
    content = content.replace(
        /                      <div>\r?\n\s*<label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center"><FiMapPin className="mr-1 text-\[var\(--color-primary\)\]"\/> Complete Address \(Agra Only\)<\/label>/g,
        email_ui
    );
}

const wallet_ui = `                        {/* Referral Display */}
                        {appliedReferral && (
...
if (!content.includes('Use Wallet Balance')) {
    let idx = content.indexOf('                        <div className="pt-3 mt-3 border-t border-pink-200 flex justify-between items-center">\n                          <span className="text-gray-800 font-bold">Final Amount</span>');
    if (idx === -1) {
        idx = content.indexOf('                        <div className="pt-3 mt-3 border-t border-pink-200 flex justify-between items-center">\r\n                          <span className="text-gray-800 font-bold">Final Amount</span>');
    }
    if (idx !== -1) {
        let target = content.substring(idx, idx + 200).split('</span>')[0] + '</span>';
...
if (!content.includes('Have a Referral Code?')) {
    ...
    let end_str = 'Coupon applied successfully!</p>\n                    </div>';
    ...
    if (idx_start !== -1) {
        ...
        if (idx_end === -1) {
             ...
             idx_end = content.indexOf(end_str, idx_start);
        }
        if (idx_end !== -1) {
            let target = content.substring(idx_start, idx_end + end_str.length);
